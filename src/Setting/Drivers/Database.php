...
namespace Nabre\Setting\Drivers;
...
class Database extends Driver
{
    ...
    {
        $userID='user_id';
        $data = $this->newQuery()->get()->filter(function($i)use($userID){
            return is_null($i[$userID]??null);
        })->values();

        $user_id = optional(auth()->user())->id;
        if (!is_null($user_id)) {
            $this->setExtraColumns(compact($userID));
            $dataUser =$this->newQuery()->get();
            $rVars=$dataUser->pluck($this->key)->toArray();
            $data=$data->reject(function($i)use($rVars){
                return in_array($i[$this->key],$rVars);
            })->merge($dataUser);
        }
